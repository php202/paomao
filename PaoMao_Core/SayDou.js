...
// ---------------------------------------------------------------------------
// 會員預約紀錄：依 membid 拉取預約歷史（供回訪提醒、預約狀態查詢）
// API: calendar/reservation/record/saydou/{saydou_id}/{membid}
// ---------------------------------------------------------------------------

/**
 * 拉一頁會員預約紀錄
 * @param {number} membid 會員 ID
 * @param {number} page 頁數 (0-based)
 * @param {number} limit 每頁筆數
 * @returns {{ items: Array, total: number, summary: Object }}
 */
function fetchMemberReservationRecordPage(membid, page, limit) {
  var bearerToken = getBearerTokenFromSheet();
  var apiUrl = 'https://saywebdatafeed.saydou.com/api/management/calendar/reservation/record/saydou/' +
    membid + '/' + membid +
    '?page=' + page + '&limit=' + (limit || 20) + '&sort=rsvtim&order=desc';
  try {
    ...
    if (code !== 200 || !json.status || !json.data) return { items: [], total: 0, summary: {} };
    return {
    ...
    Logger.log('fetchMemberReservationRecordPage exception: ' + e);
    return { items: [], total: 0, summary: {} };
  }
}

/**
 * 依 membid 全筆拉取會員預約紀錄（分頁迴圈直到取完或達上限）
 * @param {number} membid 會員 ID
 * @param {number} pageSize 每頁筆數，預設 20
 * @param {number} maxPages 最多拉幾頁，預設 5（避免過大會員拉太久）
 * @returns {{ items: Array, total: number, summary: Object }}
 */
function getAllMemberReservationRecords(membid, pageSize, maxPages) {
  if (!membid) return { items: [], total: 0, summary: {} };
  var limit = pageSize || 20;
  var maxPg = maxPages || 5;
  var all = [];
  var page = 0;
  var res;
  var summary = {};
  do {
    res = fetchMemberReservationRecordPage(membid, page, limit);
    if (page === 0) summary = res.summary;
    if (res.items && res.items.length > 0) all = all.concat(res.items);
    page++;
  } while (res.items && res.items.length >= limit && page < maxPg);
  return { items: all, total: res.total || all.length, summary: summary };
}

/**
 * 檢查會員是否有未來的有效預約（aprove="Y" 且 rsvtim >= 今天）
 * @param {number} membid 會員 ID
 * @returns {{ hasUpcoming: boolean, nextReservation: Object|null }}
 */
function checkMemberUpcomingReservation(membid) {
  if (!membid) return { hasUpcoming: false, nextReservation: null };
  var result = getAllMemberReservationRecords(membid, 20, 2);
  if (!result.items || result.items.length === 0) return { hasUpcoming: false, nextReservation: null };
  
  var today = Utilities.formatDate(new Date(), 'Asia/Taipei', 'yyyy-MM-dd');
  for (var i = 0; i < result.items.length; i++) {
    var r = result.items[i];
    // aprove = "Y" 表示已確認，memcel = null 或 false 表示會員未取消
    if (r.aprove === 'Y' && !r.memcel) {
      var rsvDate = (r.rsvtim || '').slice(0, 10);
      if (rsvDate >= today) {
        return { hasUpcoming: true, nextReservation: r };
      }
    }
  }
  return { hasUpcoming: false, nextReservation: null };
}

/**
 * 定期檢查 SayDou Bearer Token 是否仍有效。
 * 建議在 GAS 中設定時間觸發器（例如每 10~30 分鐘）執行本函式。
 * Token 有問題時會寄 Email 給腳本擁有者或 TOKEN_NOTIFY_EMAIL 指定的信箱。
 */
function checkSaydouTokenAndNotify() {
  var bearerToken = getBearerTokenFromSheet();
  if (!bearerToken) {
    try {
      var msgEmpty = "【SayDou Token 提醒】\n目前讀不到 SayDou token（可能尚未同步或尚未登入）。\n請在瀏覽器登入 https://m.saydou.com/login 一次，Tampermonkey 會自動同步新 token。";
      notifyTokenStatusByEmail(msgEmpty);
    } catch (e) {
      Logger.log("checkSaydouTokenAndNotify (no token) error: " + e);
    }
    return;
  }

  var apiUrl = "https://saywebdatafeed.saydou.com/api/management/unearn/memberStorecash" +
    "?page=0&limit=1&sort=stcash&order=desc&keyword=&showGroup=0&tabIndex=0";
  ...
  try {
    ...
    if (code === 401 || code === 403) {
      var msgExpired = "【SayDou Token 提醒】\nSayDou token 已失效。\n請在瀏覽器登入 https://m.saydou.com/login 並完成驗證，Tampermonkey 會自動同步新 token 到 GAS。";
      notifyTokenStatusByEmail(msgExpired);
    }
  } catch (e2) {
    Logger.log("checkSaydouTokenAndNotify fetch error: " + e2);
  }
}

/**
 * 取得通知 Email：優先讀 Script Properties 中的 TOKEN_NOTIFY_EMAIL，未設定則退回腳本擁有者。
 */
function getTokenNotifyEmail() {
  try {
    var props = PropertiesService.getScriptProperties();
    if (props) {
      var configured = (props.getProperty("TOKEN_NOTIFY_EMAIL") || "").trim();
      if (configured) return configured;
    }
  } catch (e) {
    Logger.log("getTokenNotifyEmail props error: " + e);
  }
  try {
    var me = (Session.getActiveUser && Session.getActiveUser().getEmail()) || "";
    return (me || "").trim();
  } catch (e2) {
    Logger.log("getTokenNotifyEmail session error: " + e2);
    return "";
  }
}
 ...
 * 寄出 Token 狀態通知 Email。
 */
function notifyTokenStatusByEmail(message) {
  var to = getTokenNotifyEmail();
  if (!to) {
    Logger.log("notifyTokenStatusByEmail skipped: no recipient email");
    return;
  }
  var subject = "【SayDou】Token 狀態通知";
