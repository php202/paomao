...
/**
 * 取得會員預約紀錄摘要（供「預約記錄」欄顯示）
 * @param {number} membid SayDou 會員 ID
 * @returns {{ summary: string, hasUpcoming: boolean, nextReservation: Object|null, items: Array }}
 */
function getMemberReservationSummary(membid) {
  if (!membid || typeof Core.getAllMemberReservationRecords !== "function") {
    return { summary: "—", hasUpcoming: false, nextReservation: null, items: [] };
  }
  try {
    var result = Core.getAllMemberReservationRecords(membid, 20, 3);
    var items = result.items || [];
    if (items.length === 0) {
      return { summary: "（無預約紀錄）", hasUpcoming: false, nextReservation: null, items: [] };
    }
    ...
    for (var i = 0; i < Math.min(items.length, maxShow); i++) {
      var r = items[i];
      var rsvDate = (r.rsvtim || "").slice(0, 10);
      var rsvTime = (r.rsvtim || "").slice(11, 16);
      var status = "";
      ...
      var services = r.services || (r.child && r.child.length > 0 ? r.child.map(function(c) { return c.services || ""; }).join(", ") : "");
      lines.push(rsvDate + " " + rsvTime + " " + (services || "—") + " " + status);
    }
    ...
    if (summary && (summary.Y || summary.N || summary.memcel)) {
      var statsLine = "統計：已確認 " + (summary.Y || 0) + " / 待確認 " + (summary.N || 0) + " / 取消 " + (summary.memcel || 0);
      lines.unshift(statsLine);
    }
    ...
    console.warn("getMemberReservationSummary:", e);
    return { summary: "（查詢失敗）", hasUpcoming: false, nextReservation: null, items: [] };
  }
}

/**
 * 依消費紀錄計算平均消費間隔（天），並建議下次回訪日
 * @param {string} phone 客人手機
 * @param {number} [membid] 可選，已有 membid 時直接傳入避免重複查詢
 * @returns {{ avgInterval: number|null, lastConsumeDate: string|null, suggestedNextVisit: string|null, daysOverdue: number|null }}
 */
function calculateSuggestedNextVisit(phone, membid) {
  var result = { avgInterval: null, lastConsumeDate: null, suggestedNextVisit: null, daysOverdue: null };
  
  if (!phone && !membid) return result;
  
  // 取得消費紀錄
  var transactions = [];
  if (membid && typeof Core.getAllTransactionsByMembid === "function") {
    try {
      transactions = Core.getAllTransactionsByMembid(membid, 20);
    } catch (e) { console.warn("getAllTransactionsByMembid:", e); }
  } else if (phone && typeof Core.getMemberRecentTransactionsForPrompt === "function") {
    try {
      transactions = Core.getMemberRecentTransactionsForPrompt(Core.normalizePhone(phone), 20);
  ...
  for (var i = 0; i < transactions.length; i++) {
    ...
    // 排除小費
    var remark = t.remark ? String(t.remark) : "";
    if (remark.indexOf("小費") >= 0) continue;
    var dateStr = t.rectim || t.cretim || t.date || "";
    if (dateStr) {
  ...
  uniqueDates.sort(function(a, b) { return b.localeCompare(a); });

  if (uniqueDates.length === 0) return result;

  result.lastConsumeDate = uniqueDates[0];

  // 計算消費間隔（需至少 2 次消費）
  if (uniqueDates.length >= 2) {
    var dayMs = 24 * 60 * 60 * 1000;
    var intervals = [];
    for (var k = 0; k < uniqueDates.length - 1 && k < 10; k++) {
      var d1 = new Date(uniqueDates[k] + "T00:00:00");
      var d2 = new Date(uniqueDates[k + 1] + "T00:00:00");
      var gap = Math.round((d1.getTime() - d2.getTime()) / dayMs);
      if (gap > 0 && gap < 365) intervals.push(gap); // 排除異常值
    }

    if (intervals.length > 0) {
      // 計算平均間隔
      var sum = 0;
      for (var m = 0; m < intervals.length; m++) sum += intervals[m];
      result.avgInterval = Math.round(sum / intervals.length);

      // 計算建議下次回訪日
      var lastDate = new Date(result.lastConsumeDate + "T00:00:00");
      var suggestedDate = new Date(lastDate.getTime() + result.avgInterval * dayMs);
      result.suggestedNextVisit = Utilities.formatDate(suggestedDate, "Asia/Taipei", "yyyy-MM-dd");

      // 計算是否已超過建議日期
      var today = new Date();
      var todayStr = Utilities.formatDate(today, "Asia/Taipei", "yyyy-MM-dd");
      if (result.suggestedNextVisit < todayStr) {
        var overdue = Math.round((today.getTime() - suggestedDate.getTime()) / dayMs);
        result.daysOverdue = overdue;
      }
    }
  }
...
/**
 * 格式化「建議下次回訪日」欄位顯示文字
 * @param {Object} visitInfo calculateSuggestedNextVisit 的回傳值
 * @param {boolean} hasUpcomingReservation 是否已有預約
 * @returns {string}
 */
function formatSuggestedNextVisitColumn(visitInfo, hasUpcomingReservation) {
  if (!visitInfo || !visitInfo.suggestedNextVisit) {
    if (visitInfo && visitInfo.lastConsumeDate) {
      return "上次消費 " + visitInfo.lastConsumeDate + "（消費次數不足，無法計算）";
    }
    return "（無消費紀錄）";
  }

  var lines = [];
  lines.push("上次消費：" + visitInfo.lastConsumeDate);
  lines.push("平均間隔：" + visitInfo.avgInterval + " 天");
  lines.push("建議回訪：" + visitInfo.suggestedNextVisit);

  if (hasUpcomingReservation) {
    lines.push("★ 已有預約，免通知");
  } else if (visitInfo.daysOverdue != null && visitInfo.daysOverdue > 0) {
    lines.push("⚠ 已超過 " + visitInfo.daysOverdue + " 天");
  }
...
function buildIntegratedRow(phone, timestamp, currentSubmission, existingLineUserId) {
  ...
  // 預約記錄與建議下次回訪日
  var reservationSummary = getMemberReservationSummary(saydouSummary.member && saydouSummary.member.membid);
  var visitInfo = calculateSuggestedNextVisit(phone, saydouSummary.member && saydouSummary.member.membid);
  var suggestedNextVisitText = formatSuggestedNextVisitColumn(visitInfo, reservationSummary.hasUpcoming);
  ...
  return [
    ...
    "",  // AI分析結果（由 AI 戰報函式寫入）
    "",  // ai調整建議（由員工填寫）
    reservationSummary.summary,     // 預約記錄
    suggestedNextVisitText          // 建議下次回訪日
  ];
}
...
/** 回訪提醒配置 */
var REVISIT_REMINDER_CONFIG = {
  /** 預設提醒訊息模板（{lastDate}=上次消費日, {daysAgo}=距今天數, {avgInterval}=平均間隔） */
  MESSAGE_TEMPLATE: "您好，您上次來的時間{lastDate}已經過了{daysAgo}天，今天已經到你習慣的保養時間囉～\n點選下方可以幫您做預約消費。",
  /** 超過建議日幾天開始提醒（0 = 當天） */
  REMIND_AFTER_DAYS: 0,
  /** 最大提醒超過天數（超過太久不提醒，可能已流失） */
  MAX_OVERDUE_DAYS: 30
};

/**
 * 掃街檢查需要回訪提醒的客人，並發送 Push 通知
 * 條件：
 * 1. 有 lineUserId（可以 Push）
 * 2. 有建議下次回訪日，且已到期或超過
 * 3. 沒有已預約的未來預約
 * @returns {{ scanned: number, reminded: number, skipped: number, errors: number }}
 */
function scanAndSendRevisitReminders() {
  console.log("scanAndSendRevisitReminders: 開始掃街");
  
  var ss = SpreadsheetApp.openById(CONFIG.INTEGRATED_SHEET_SS_ID);
  var sheet = getOrCreateIntegratedSheet(ss);
  var lastRow = sheet.getLastRow();
  var headers = CONFIG.INTEGRATED_HEADERS;
  
  var phoneCol = headers.indexOf("手機") + 1;
  var lineUserIdCol = headers.indexOf("lineUserId") + 1;
  var suggestedNextVisitCol = headers.indexOf("建議下次回訪日") + 1;
  var reservationCol = headers.indexOf("預約記錄") + 1;
  var saydouIdCol = headers.indexOf("saydouUserId") + 1;
  
  if (phoneCol < 1 || lineUserIdCol < 1 || suggestedNextVisitCol < 1) {
    console.warn("scanAndSendRevisitReminders: 找不到必要欄位");
    return { scanned: 0, reminded: 0, skipped: 0, errors: 0 };
  }
  
  var dataStartRow = (lastRow >= 1 && sheet.getRange(1, lineUserIdCol).getValue() === "lineUserId") ? 2 : 1;
  if (lastRow < dataStartRow) {
    console.log("scanAndSendRevisitReminders: 無資料列");
    return { scanned: 0, reminded: 0, skipped: 0, errors: 0 };
  }
  ...
  for (var r = dataStartRow; r <= lastRow; r++) {
    ...
    try {
      ...
      var suggestedNextVisitRaw = sheet.getRange(r, suggestedNextVisitCol).getValue();
      var reservationRaw = sheet.getRange(r, reservationCol).getValue();
      
      // 1. 沒有 lineUserId 則跳過
      if (!lineUserIdRaw || String(lineUserIdRaw).trim() === "") {
        skipped++;
        continue;
      }
      
      // 2. 解析建議下次回訪日欄位
      var suggestedText = String(suggestedNextVisitRaw || "");
      var suggestedDateMatch = suggestedText.match(/建議回訪[：:]\s*(\d{4}-\d{2}-\d{2})/);
      if (!suggestedDateMatch) {
        skipped++;
        continue;
      }
      var suggestedDate = suggestedDateMatch[1];
      ...
      // 4. 檢查是否超過太久（可能已流失）
      var overdueDays = Math.round((new Date(today + "T00:00:00").getTime() - new Date(suggestedDate + "T00:00:00").getTime()) / dayMs);
      if (overdueDays > REVISIT_REMINDER_CONFIG.MAX_OVERDUE_DAYS) {
        skipped++;
        continue;
      }
      
      // 5. 檢查是否已有預約（欄位含「已確認✓」表示有未來預約）
      var reservationText = String(reservationRaw || "");
      if (reservationText.indexOf("已確認✓") >= 0) {
        skipped++;
        continue;
      }
      
      // 6. 額外確認：透過 API 檢查是否有未來預約（更準確）
      var saydouId = sheet.getRange(r, saydouIdCol).getValue();
      if (saydouId && saydouId !== "—" && typeof Core.checkMemberUpcomingReservation === "function") {
        try {
          var upcomingCheck = Core.checkMemberUpcomingReservation(Number(saydouId));
          if (upcomingCheck && upcomingCheck.hasUpcoming) {
            skipped++;
            continue;
          }
        } catch (e) {
          console.warn("checkMemberUpcomingReservation 失敗: " + (e && e.message));
        }
      }
      
      // 7. 解析上次消費日期
      var lastDateMatch = suggestedText.match(/上次消費[：:]\s*(\d{4}-\d{2}-\d{2})/);
      var lastDate = lastDateMatch ? lastDateMatch[1] : "";
      var daysAgo = lastDate ? Math.round((new Date(today + "T00:00:00").getTime() - new Date(lastDate + "T00:00:00").getTime()) / dayMs) : 0;
      
      // 8. 組合訊息
      var message = REVISIT_REMINDER_CONFIG.MESSAGE_TEMPLATE
        .replace("{lastDate}", lastDate.slice(5).replace("-", "/"))  // 轉成 MM/DD 格式
        .replace("{daysAgo}", String(daysAgo))
        .replace("{avgInterval}", "");
      
      // 9. 發送 Push（使用客人的 LINE）
      var userIds = parseLineUserIds(String(lineUserIdRaw));
      if (userIds.length === 0) {
        skipped++;
        continue;
      }
      
      // 發送給第一個有效的 userId（客人帳號）
      var sentOk = false;
      for (var u = 0; u < userIds.length; u++) {
        var userId = userIds[u];
        if (userId && typeof Core.sendLinePushText === "function") {
          try {
            // 取得客人用的 LINE Token（非員工打卡用）
            var config = typeof Core.getCoreConfig === "function" ? Core.getCoreConfig() : null;
            var lineToken = config && config.LINE_TOKEN_PAOBUBBLE ? config.LINE_TOKEN_PAOBUBBLE : null;
            if (lineToken) {
              Core.sendLinePushText(userId, message, lineToken);
              sentOk = true;
              console.log("已發送回訪提醒: " + phone + " → " + userId);
              break;
            }
          } catch (e) {
            console.warn("sendLinePushText 失敗: " + (e && e.message));
          }
        }
      }
      ...
      console.warn("scanAndSendRevisitReminders 列 " + r + " 失敗: " + (e && e.message));
      errors++;
    }
  }
  
  console.log("scanAndSendRevisitReminders: 掃描 " + scanned + " 筆，已提醒 " + reminded + " 筆，跳過 " + skipped + " 筆，失敗 " + errors + " 筆");
  return { scanned: scanned, reminded: reminded, skipped: skipped, errors: errors };
}

/**
 * 手動執行回訪提醒（測試用）
 * 注意：正式環境請設定排程觸發，避免重複發送
 */
function runRevisitRemindersManual() {
  console.log("runRevisitRemindersManual: 開始手動執行回訪提醒");
  var result = scanAndSendRevisitReminders();
  console.log("runRevisitRemindersManual: 完成 - " + JSON.stringify(result));
  return result;
}
